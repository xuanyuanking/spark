...
//      if (curField.dataType.isInstanceOf[StructType]) {
//        val newField = StructType(curField.dataType.asInstanceOf[StructType].fields)
//          .apply(nextFieldStr)
//        StructField(curField.name, StructType(Seq(newField)),
//          curField.nullable, curField.metadata)
//      } else throw new IllegalArgumentException(s"""Field "$curFieldStr" is not struct field.""")
